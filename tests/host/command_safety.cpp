...
static void test_network_info() {
    char ip[16];
    assert(format_ipv4(ip, sizeof(ip), 192, 0, 2, 42));
    assert(strcmp(ip, "192.0.2.42") == 0);
...
static void test_input_state_filter() {
    InputStateFilter filter(DRIVE_INPUT_FILTER_DURATION_MS);
    filter.reset(DOORSTATUSOPEN, 1000);
    assert(filter.update(DOORSTATUSOPEN, 1000) == DOORSTATUSOPEN);

    // A 439 us / sub-millisecond external transition at the end switch must
    // not affect the state consumed by the controller.
    assert(filter.update(DOORSTATUSEXTERNAL, 1010) == DOORSTATUSOPEN);
    assert(filter.update(DOORSTATUSOPEN, 1011) == DOORSTATUSOPEN);

    assert(filter.update(DOORSTATUSMOVINGORSTOPPED, 1020) == DOORSTATUSOPEN);
    assert(filter.update(DOORSTATUSMOVINGORSTOPPED, 1039) == DOORSTATUSOPEN);
    assert(filter.update(DOORSTATUSMOVINGORSTOPPED, 1040) == DOORSTATUSMOVINGORSTOPPED);

    // Samples from the controller's own output pulse must never settle into a
    // door state after the pulse has ended.
    filter.reset(DOORSTATUSCLOSED, 2000);
    filter.discard(DOORSTATUSOPEN, 2010);
    assert(filter.update(DOORSTATUSMOVINGORSTOPPED, 2011) == DOORSTATUSCLOSED);
    assert(filter.update(DOORSTATUSMOVINGORSTOPPED, 2031) == DOORSTATUSMOVINGORSTOPPED);

    // The duration calculation must retain the same behavior across wrap.
    filter.reset(DOORSTATUSCLOSED, UINT32_MAX - 10);
    assert(filter.update(DOORSTATUSOPEN, UINT32_MAX - 5) == DOORSTATUSCLOSED);
    assert(filter.update(DOORSTATUSOPEN, 13) == DOORSTATUSCLOSED);
